...
test_that("nonlinear Gibbs/BF gives identical results as A*x<b version",{
  ...
  expect_silent(bf_r <- bf_nonlinear(k, opt, mod, M = 20000, progress = FALSE))

  ############# A*x < b
  A <- matrix(c(1, -1,
                1, 2), 2,2, TRUE)
  b <- c(0,1)
  mcmc_Ab <- sampling_multinom(k, opt, A, b, M=25000, progress = FALSE)
  ...
  expect_silent(mcmc_cpp <- sampling_nonlinear(k, opt, mod_ptr, M = 25000,
                                               progress = FALSE))
  expect_gt(ks.test(mcmc_Ab[,1], mcmc_cpp[,1])$p, .01)
