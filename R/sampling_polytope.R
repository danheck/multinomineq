...
#' binomial and multinomial models with linear inequality-constraints.
#'
...
#' @return a matrix with posterior samples for the multinomial probability parameters.
...
sampling_multinom <- function (k, options, A, b, V, prior = rep(1, sum(options)),
  ...
  if (!missing(V)){
    samples <- sampling_V(k, options, V, prior = prior, M = M,
...
sampling_binom <- function (k, n, A, b, V, map = 1:ncol(A), prior = c(1, 1),
                            M = 5000, start, burnin = 10, progress = TRUE){
  if (!missing(V)){
    samples <- sampling_V(k, options = rep(2, length(k)), V = V,
