...
#'    the last category for each option (cf. \code{p_drop}).
...
#' @param options number of categories/probabilities for each option/multinomial distribution.
...
rpbinom <- function(prob, n){
  k <- rpmultinom(prob = prob, n = n,
                  options = rep(2, length(prob)), p_drop = TRUE)
  drop_fixed(k, options = 2)
}

#' @rdname rpbinom
#' @export
rpmultinom <- function(prob, n, options, p_drop = TRUE){
  if (length(n) == 1)
    n <- rep(n, sum(options))
  if (is.null(dim(prob)))
    prob <- t(prob)
  check_probko(prob = prob, k = rep(0, sum(options)),
               options = options, p_drop = p_drop)

  if (p_drop) prob <- add_fixed(prob, options = options)
